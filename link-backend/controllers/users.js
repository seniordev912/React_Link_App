...
module.exports = {
	...
	addView: async (req, res) => {
		...
		const existsSession = await Card.findOne({ _id: cardId }, '_id').populate({
			path: 'views',
			match: {
				$or: [
					{
		...
		console.log(existsSess)
		console.log(existsSession)
		if (existsSession.views.length !== 0) {
			return res
				.status(400)
				.send(
	...
	getUser: async (req, res) => {
		...
		if (!userBase) {
			return res.status(200).json({ result: false, error: `This user doesn't exist.` })
		}
	...
	checkUser: async (req, res) => {
		...
		if (userBase) {
			return res.status(200).json({ result: true, message: `This user exists` })
		} else {
			return res.status(200).json({ result: false, message: `This user doesn't exist.` })
		}
	},
	...
	clickButton: async (req, res) => {
		...
		const currentDate = new Date()
		const existsSession = await Button.findOne(
			{ _id: buttonId },
			'_id'
		).populate({
			path: 'clicks',
		...
		sessionObj.count = existsSession.clicks.length === 0
		if (geo) {
			const exists = await GeoInfo.findOne(
				{ ip, device: req.device.type },
				'_id'
	...
	clickSocial: async (req, res) => {
		...
		const currentDate = new Date()
		const existsSession = await Social.findOne(
			{ _id: socialId },
			'_id'
		).populate({
			path: 'clicks',
