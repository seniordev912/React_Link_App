...
class NSignup extends Component {
	...
	onFinishStep1 = async values => {
		const { step, street_num, city, province, zipcode, countryValue, product } = this.state
		...
		try{
			...
			if(step1_res.data.success) {
				if(product && product.ship) {
					...
					iziToast.error({
						title: 'Error',
						message: "Cannot read product info"
					})
				}
			} else {
				iziToast.error({
		...
		return (
			<div className="container-fluid card-main-container">
				...
				<Row className="step-container">
						...
						<>
							...
							<Col className="s-right-part" lg={16} xs={24} md={24}>
								...
								{/* <div className="skip-step-part">
									<a onClick={this.skipCardStep}>Skip this step</a>
								</div> */}
							</Col>
						</>
					)}
					...
					{/* {step === 2 && (
						<SignUp paymentInfos={paymentInfos} userInfos={userInfos} />
					)} */}
				</Row>
			</div>
		)
